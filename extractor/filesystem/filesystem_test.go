...
func TestRun_EmbeddedFS(t *testing.T) {
	...
	for _, p := range gotInv.Packages {
		sort.Slice(p.Location.Related, func(i, j int) bool {
			return p.Location.Related[i].File.Path < p.Location.Related[j].File.Path
		})
	}
...
// fakeExtractorRunningSystem is a mock extractor that requires a running system.
type fakeExtractorRunningSystem struct {
	name string
}

func (e *fakeExtractorRunningSystem) Name() string { return e.name }
func (e *fakeExtractorRunningSystem) Version() int { return 1 }
func (e *fakeExtractorRunningSystem) Requirements() *plugin.Capabilities {
	return &plugin.Capabilities{RunningSystem: true}
}
func (e *fakeExtractorRunningSystem) FileRequired(api filesystem.FileAPI) bool {
	path := filepath.ToSlash(api.Path())
	return strings.HasSuffix(path, "file.txt") || strings.HasSuffix(path, "/file.txt") || path == "file.txt" || path == "./file.txt"
}
func (e *fakeExtractorRunningSystem) Extract(ctx context.Context, input *filesystem.ScanInput) (inventory.Inventory, error) {
	path := filepath.ToSlash(input.Path)
	return inventory.Inventory{
		Packages: []*extractor.Package{
			{
				Name:     "RunningSystemSoftware",
				Location: extractor.LocationFromPath(path),
				Plugins:  []string{e.Name()},
			},
		},
...
func TestRun_EmbeddedFS_RunningSystem(t *testing.T) {
	...
	fakeExSoftware := &fakeExtractorSoftware{name: "fake-ex-software"}
	fakeExRunningSystem := &fakeExtractorRunningSystem{name: "fake-ex-runningsystem"}
	...
	if diff := cmp.Diff(wantInv, gotInv, cmpopts.SortSlices(extracttest.PackageCmpLess), fe.AllowUnexported, cmp.AllowUnexported(fakeExtractorFS{}, fakeExtractorSoftware{}, fakeExtractorRunningSystem{}), cmpopts.EquateErrors(), cmpopts.IgnoreFields(inventory.EmbeddedFS{}, "GetEmbeddedFS")); diff != "" {
		t.Errorf("filesystem.Run(%v): unexpected findings (-want +got):\n%s", config, diff)
	}
}
	...
	testCases := []struct {
	...
	}{
		{
			...
			wantPkg: inventory.Inventory{Packages: []*extractor.Package{
				{
					Name:     name1,
					Location: extractor.LocationFromPath(path1),
					Plugins:  []string{fakeEx1.Name()},
				},
				{
					Name:     name2,
					Location: extractor.LocationFromPath(path2),
		...
		},
		{
			...
			wantPkg: inventory.Inventory{Packages: []*extractor.Package{
				...
				{
					Name:     name2,
					Location: extractor.LocationFromPath(dir1),
